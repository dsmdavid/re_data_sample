{% macro create_table_dq_issue_log() %}
...
{% elif target.type | lower == 'bigquery' %}
  {% set query %}
    create table if not exists {{ target.project }}.{{ target.dataset }}.dq_issue_log
    (
...
{% elif execute 
  and flags.WHICH in ['run', 'test', 'compile'] 
  and (target.name | upper).startswith('PR_CI') %}
